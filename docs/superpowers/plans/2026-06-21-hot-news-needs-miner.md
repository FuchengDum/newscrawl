...
- [ ] **Step 1: 编写爬虫脚本 crawler.py**
    ...
    def fetch_weibo_hot():
        ...
        try:
            ...
            return results, False  # 数据，是否降级
        except Exception as e:
            # 回退至公网 RSSHub 镜像源
            return fetch_weibo_rss_fallback()

    def fetch_weibo_rss_fallback():
        rss_url = "https://rsshub.app/weibo/search/热搜"
        try:
            # 引入 bs4 解析 RSS
    ...
    def fetch_zhihu_hot():
        ...
        try:
            ...
            return fetch_zhihu_rss_fallback()

    def fetch_zhihu_rss_fallback():
        ...
        try:
            ...
            for idx, item in enumerate(items[:50]):
                ...
                results.append({"title": title, "url": link, "platform": "zhihu", "popularity": 100000 - idx})
            return results, True
        except Exception:
            return [], True
