...
This document outlines the design to move hardcoded API configurations (endpoints, keys, and models) from `ai_analyst.py` into environment variables, loadable via `.env`.
...
- The following configurations will be retrieved:
  ...
  - `FALLBACK_API_URL`
  - `FALLBACK_API_KEY`
  - `FALLBACK_MODELS` (parsed as a list of strings from comma-separated format)
